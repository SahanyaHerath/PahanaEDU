    ...
    <div class="lg:ml-64">
        <div class="bg-white shadow-sm border-b border-gray-200">
            <div class="flex items-center justify-between px-6 py-4">
                <div class="flex items-center space-x-4"><button onclick="toggleSidebar()" class="lg:hidden text-gray-500 hover:text-gray-700"><i class="fas fa-bars text-xl"></i></button><h1 class="text-2xl font-bold text-gray-900">Product Details</h1></div>
        ...
        <div class="p-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 border-b pb-4">
                    ...
                    <span class="inline-flex items-center bg-primary-600 text-white px-3 py-1 rounded-full text-sm">ID: ${product.productId}</span>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <div class="text-gray-600 text-sm mb-1">Price</div>
                        <div class="text-xl font-semibold text-gray-900"><fmt:formatNumber value="${product.price}" type="currency" currencySymbol="Rs. "/></div>
                    </div>
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    ...
                    <div class="text-gray-800 whitespace-pre-line min-h-[80px]"><c:choose><c:when test="${not empty product.description}">${product.description}</c:when><c:otherwise><em class="text-gray-500">No description available.</em></c:otherwise></c:choose></div>
                </div>

                <div class="flex items-center gap-3 mt-8 justify-center">
                    <a href="product?action=edit&id=${product.productId}" class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors duration-200"><i class="fas fa-edit mr-2"></i>Edit Product</a>
                    <a href="product?action=delete&id=${product.productId}" onclick="return confirm('Are you sure you want to delete this product?')" class="bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700 transition-colors duration-200"><i class="fas fa-trash mr-2"></i>Delete</a>
                    <a href="product?action=list" class="bg-gray-100 text-gray-700 px-6 py-3 rounded-lg hover:bg-gray-200 transition-colors duration-200"><i class="fas fa-arrow-left mr-2"></i>Back</a>
                </div>
            </div>
        </div>
    </div>
